...
class ComposeActivity : BaseActivity(),
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (loadInstanceData(preferences)) {
    ...
    private fun loadInstanceData(preferences: SharedPreferences): Boolean {
        if (composeOptions?.tootRightNow == true) {
    ...
    private fun setupDefaultTagViews(preferences: SharedPreferences) {
        checkboxUseDefaultText.isChecked = preferences.getBoolean(PREF_USE_DEFAULT_TAG, false)
        checkboxUseDefaultText.setOnCheckedChangeListener { _, isChecked ->
            preferences.edit()
        ...
        editTextDefaultText.setText(preferences.getString(PREF_DEFAULT_TAG, ""))
        editTextDefaultText.doAfterTextChanged {
            preferences.edit()
    ...
    private fun setupButtons() {
        ...
        composeOptionsBottomSheet.allowUnleakable(viewModel.domain in CAN_USE_UNLEAKABLE)
    ...
    fun calculateTextLength(): Int {
        ...
        if (checkboxUseDefaultText.isChecked) {
            length += 1 + editTextDefaultText.length()
        }
